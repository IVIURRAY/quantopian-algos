...
def decision(context, data):
    """
    Decide weather to buy or sell a security.
    """
    # Get the 30-day price history for each security in our list.
    hist = data.history(context.security_list, 'price', 31, '1d')

    # Create 10-day and 30-day trailing windows. Ignoere todays price.
    prices_10 = hist[-11:-1]
    prices_30 = hist[:-1]
    ...
    if sma_10.mean() > sma_30.mean():
        decision = 1
    elif sma_10.mean() == sma_30.mean():
        decision = 0
    else:
        decision = -1
...
def rebalance(context, data):
    ...
    for security in context.security_list:
        ...
        if data.can_trade(security) and security not in open_pos:
            if dec > 0:
                order_target_percent(security, dec)
            elif dec == 0:
                order_target_percent(security, dec)
            else:
                order_target_percent(security, dec)
