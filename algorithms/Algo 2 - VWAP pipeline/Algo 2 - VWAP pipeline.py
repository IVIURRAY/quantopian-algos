...
def vwap_cross(context, data):
    """if price was below vwap yesterday but above today then buy"""
    spy = context.output.index[0]
    
    d = data.history(spy, ['price', 'high', 'low', 'volume'], 2, '1d')
    #new = data.history(spy, ['price', 'high', 'low', 'volume'], 1, '1d')
    
    today = d.index[-1]
    yesterday = d.index[-2]
    old = d.filter(like=str(yesterday.date()), axis=0)
    new = d.filter(like=str(today.date()), axis=0)
    
    def _vwap(dd):
        v = dd.get('volume')
        h = dd.get('high')
        l = dd.get('low')
        return ((v * (h+l)/2 ).cumsum() / v.cumsum()).sum() 
    
    today_vwap        = _vwap(new)
    yesterday_vwap    = _vwap(old)
    today_price = d['price'][-1]
    yesterday_price = context.output['close'][0]
    
    if (yesterday_price > yesterday_vwap) and (today_price < today_vwap):
        return True
...
def rebalance(context,data):
    ...
    uptrend = vwap_cross(context, data)
...
def handle_data(context,data):
    """
    Called at the end of every minute.
    """
    x = '1'
    pass
